...
def verify(a, b, bijection, f1, f2):
    global elements
    assert a in elements, str(a) + " is not an element of G."
    assert b in elements, str(b) + " is not an element of G."
    return bijection[f1(a,b)] == f2(bijection[a],bijection[b])

def testBijection(bijection, f1, f2):
    failures = []
    for a in bijection.keys():
        for b in bijection.keys():
            if not verify(a, b, bijection, f1, f2):
                failures.append(a + ' = ' + bijection[a] + ' and ' + b + ' = ' + bijection[b] + ' failed:\n'
                                + '    f('+a+', '+b+') = ' +f(a, b) + ' but f2('+bijection[a]+','+bijection[b]+ ') = ' + f2(bijection[a],bijection[b])+".\n")
    return failures
